...
void clean_graph(
    ...
    string& path_to_minimap){

    int nb_of_deleted_contigs = 0;
    int length_of_deleted_contigs = 0;
    string gfaFile;
    string fastaFile;
    string format;
    ...
    while (std::getline(sam, line)){
        ...
        if (cigarStart < 0.1*lengthOfContig && cigarEnd < 0.1*lengthOfContig){
            ...
            if (contigNeighbors[contigs[nameOfContig]].first.size() == 0 && contigNeighbors[contigs[nameOfContig]].second.size() == 0){
                ...
                //check if the contig has the same neighbor as the contig it maps on
            ...
            if (disconnected){
                log << "Suppressing contig " << nameOfContig << " because it is disconnected from the rest of the graph and contained in " << contigItMapsOn << endl;
            }
        }
...
int main(int argc, char *argv[]){
    if (argc < 5){
        cout << "Usage: clean_graph <assemblyFile> <newAssembly> <outputFolder> <logFile> <threads> <path_to_minimap>" << endl;
        cout << "assemblyFile: the assembly graph in gfa or fasta format" << endl;
        cout << "newAssembly: the cleaned assembly graph in gfa or fasta format" << endl;
        cout << "outputFolder: the folder where the temporary files will be stored" << endl;
        cout << "logFile: the log file" << endl;
        cout << "threads: the number of threads to use" << endl;
        cout << "path_to_minimap: the path to the minimap executable" << endl;
        return 1;
    }
    ...
    string log = argv[4];
    string path_to_minimap = argv[6];
    clean_graph(assemblyFile, newFile, log, std::stoi(argv[5]), outputFolder, path_to_minimap);

    //TODO : include reassemble_unaligned_reads here
